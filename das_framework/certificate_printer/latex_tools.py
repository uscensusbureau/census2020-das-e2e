...
def run_latex(infile,repeat=1,start_run=1,delete_tempfiles=False,
    ...
    for i in range(start_run,start_run+repeat):
        ...
        r = subprocess.run(cmd,stdout=PIPE,stderr=PIPE,stdin=DEVNULL,shell=False)
        if r.returncode and not ignore_ret:
            outlines = r.stdout.decode('utf-8').split("\n")
            print("***************************")
        ...
        if r.returncode and DEBUG:
            ...
            print("STDOUT")
            print(r.stdout.stdout.decode('utf-8'))
            print("STDERR")
            print(r.stderr.stdout.decode('utf-8'))
    #
